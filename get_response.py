...
def concat_messages(conversations: List[Dict[str, str]], role: str, system: str) -> Tuple[List[Dict[str, str]], str]:
    """Convert raw dialogue context into user/assistant message pairs."""
...
def to_chatglm_history(messages: List[Dict[str, str]]):
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--model_name", required=True, help="模型名称（用于区分输出文件名）")
    ...
    parser.add_argument("--output_path", type=Path, default=Path("results"), help="生成结果输出目录")
...
def main() -> None:
    ...
    output_file = Path(args.output_path) / f"generation_{args.model_name}.jsonl"
