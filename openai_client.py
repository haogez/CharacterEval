"""
OpenAI API 客户端封装模块与角色生成逻辑（新版 Prompt 已完全重构，无 relationship_to_protagonist 字段）。
"""
    ...
    async def generate_structured_response(self, system_prompt: str, user_prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if json_match:
                ...
                print("尝试从 ```json``` 代码块中解析 JSON")
                parsed = json.loads(candidate)
                print("✓ JSON 解析成功（代码块）")
                return parsed
        except Exception as e:
            print(f"✗ 从 ```json``` 中提取失败: {e}")
        ...
        try:
            ...
            if json_match:
                candidate = json_match.group(1).strip()
                print("尝试从文本中截取第一个大括号块解析 JSON")
                parsed = json.loads(candidate)
                print("✓ JSON 解析成功（截取大括号）")
                return parsed
        except Exception as e:
            print(f"✗ 从大括号块中解析失败: {e}")

        print("✗ 无法提取有效 JSON，返回原文以便排查")
...
class CharacterLLM:
    ...
    async def generate_character(
        ...
        *,
        ...
        """
        根据一段 description 生成完整 20 维角色档案。
        timeline_mode:
          - "strict": past_experience 为关键事件，background 逐年成长史（你现在主要用这个）
          - "relaxed": 时间线可以按阶段描述
        """
