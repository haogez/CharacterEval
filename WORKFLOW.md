...
## 5. 生成对话回复（多模型模式，当前支持 ChatGLM3）
- `get_response.py` 现在支持通过 `--model_name` 区分输出文件名，便于对比不同模型的生成结果。
- 使用 ChatGLM3 并加载新档案：
  ```bash
  CUDA_VISIBLE_DEVICES=0 python get_response.py \
    ...
    --output_path results
  ```
- 输出文件将命名为 `results/generation_chatglm3.jsonl`。若未提供生成档案，脚本会自动回退到 `data/character_profiles.json`。
- 未来测试其他模型（如 Qwen、DeepSeek、Llama、OpenAI）时，只需将 `--model_name` 和 `--model_path` 改为对应模型，输出文件会自动使用新的模型名后缀；当前代码仅实现了 ChatGLM 分支，其他模型可按需在脚本中添加对应加载逻辑。
